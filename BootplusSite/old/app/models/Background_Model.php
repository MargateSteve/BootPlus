...
class Background_Model extends Functions
{
    ...
    {   $data = '';
        ...
        foreach ($this->get_example_settings() as $value) {
			...
			{
				$data .= '<div class="row">';
				$data .= '<div class="col-md-2">bg-'.$value.'-xxx</div>';
				...
				foreach ($this->get_palette_numbers() as $level)
				{
					...
					$data .= Html::span (
		                $level,
		                array (
		                    'class' => 'bg-'.$value.'-'.$level.' font-weight-bold mr-2 p-1'
		                )
		            );
                    ...
                    $data .= Html::span (
		                $level,
		                array (
		                    'class' => 'bg-'.$value.'-'.$level.' contrast font-weight-bold mr-2 p-1'
		                )
		            );
                    ...
                    $data .= Html::span (
		                $level,
		                array (
		                    'class' => 'bg-'.$value.'-'.$level.' compliment font-weight-bold mr-2 p-1'
		                )
		            );
					...
					$data .= Html::span (
		                $level,
		                array (
		                    'class' => 'bg-'.$value.'-'.$level.' compliment_light font-weight-bold mr-2 p-1'
		                )
		            );
                    $data .=  '<br>';
				}
